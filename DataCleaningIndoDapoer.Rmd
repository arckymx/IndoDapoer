...
Use the following tutorial: Intro to R Session 3.5 (Data Manipulation - World Bank Data) [https://www.youtube.com/watch?v=6qmYxYtY6Rg]
...
## Subsetting
subsetting the data based on the following criteria selection (1) provincial scale, (2) TS 2000-2012, (3) 52 variables of choice. *Tip: Use MS-Excel 'VLOOKUP' and cell merge function '&' to simplifly the scripting process.
```{r, eval=FALSE}
...
Currently, the data frame should have the following column names
```{r, eval=FALSE}
names(subset3)
```
 [1] "Country.Name"  
...
## (1) Melting data
The next step is to melt the data, in order to get the following columns: [1]"Province", [2]"Indicator", [3]"Code", [4]"Year"  [5]"Value"
```{r, eval=FALSE}
...
```
By now the data should show the following column names, when using the **names(mymelt)** function:
```{r, eval=FALSE}
names(mymelt)
```
[1]"Province", [2]"Indicator", [3]"Code", [4]"Year"  [5]"Value"
...
Using 'dcast' to reshape the data to show the following columns: Province~IndicatorYear (e.g. Jakarta, GDP 2010, GDP 2011, and so on)
```{r, eval=FALSE}
# Create a new dataframe that clones subset 3
subset4 <- data.frame(mymelt)

# Create a  column on the clone that combines "Indicator" and "Year"
subset4$Indicator <- paste(subset4$Indicator, subset4$Year, sep="")

# Examine the result
head(subset4)

# Cast the dataframe to reflect Province~Indicator(+Year)
mycast2 <- dcast(subset4,Province~Indicator)

Examine the result
head(mycast2)

# Export the data
write.csv(mycast2, "dapoerSPSS.csv")
```
